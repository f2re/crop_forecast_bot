...
## Текущий вертикальный срез — ручной обзор погодных рисков

Статус: **реализуется в PR #39; новые формулы и пороги не вводятся**.

### Причина

Scheduler уже рассчитывает пять ансамблевых рисков, но пользователь не мог самостоятельно запросить единый текущий обзор для активного поля.
...
- сообщение укладывается в лимит Telegram 4096 символов.

### Критерии готовности

- полный Dispatcher-flow `поле → культура → ручной обзор рисков`;
- unit/contract tests расчёта и форматтера;
- PostgreSQL/Redis integration, backup/restore и Alembic graph зелёные;
- live provider contracts не ухудшены;
- нет claims вероятности ущерба или града.

## Следующий вертикальный срез — журнал и тренд риска

Приоритет: **P1, сразу после PR #39**.
