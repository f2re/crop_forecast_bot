...
Текущий вертикальный срез: **ансамблевый мониторинг погодных рисков NOAA GFS через Open-Meteo**. Код, unit/contract tests и методика добавлены в отдельной ветке; полный CI и внешняя полевая приёмка ещё обязательны.
...
- HTC/ГТК остаётся ограниченным индикатором: до непрерывного сезонного окна и региональной проверки его нельзя трактовать как полноценную сезонную засуховую классификацию.
...
- Урожайность, дозы удобрений и препаратов не прогнозируются без проверяемой модели/норматива.
...
- [ ] сравнение GFS/ERA5-Land/ET₀ с локальными станциями;
...
- [ ] исправить сезонную непрерывность HTC/ГТК и скрывать результат при неполном покрытии;
...
- [ ] провести station comparison минимум в нескольких регионах/сезонах.
...
**Code-level:** основной бот и новый ансамблевый срез реализованы.
