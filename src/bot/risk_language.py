...
_ACTIONS: dict[RiskType, dict[RiskLevel, str]] = {
    "frost": {
        ...
        "elevated": "Подготовьте укрытия и проверьте полевой датчик.",
        "high": "Защитите чувствительные посадки и контролируйте температуру ночью.",
    },
    "heat": {
        "watch": "Проверьте влагу почвы и состояние растений.",
        "elevated": "Подготовьте полив и перенесите чувствительные работы с полудня.",
        "high": "Контролируйте влагу и перенесите работы из самых жарких часов.",
    },
    "heavy_rain": {
        "watch": "Проверьте водоотвод и перепроверьте прогноз завтра.",
        "elevated": "Очистите водоотвод и не планируйте работы на сырой почве.",
        "high": "Обеспечьте водоотвод и перенесите работы с техникой.",
    },
    "strong_wind": {
        "watch": "Пока только наблюдайте; перепроверьте прогноз завтра.",
        "elevated": "Проверьте крепления теплиц, укрытий и оборудования.",
        "high": "Закрепите теплицы и оборудование; перенесите опрыскивание.",
    },
    "convection": {
        "watch": "Следите за официальными предупреждениями и радаром.",
        "elevated": "Закрепите оборудование и подготовьте безопасное укрытие.",
        "high": "При предупреждении уберите людей и технику с открытых участков.",
    },
}
_MONTHS = (
    "",
...
def format_risk_period(period: RiskPeriod) -> str:
    ...
    qualifier = " · не прогноз грозы" if period.risk_type == "convection" else ""

    return (
        f"{emoji} <b>{name} — {status}</b>\n"
        f"{period_date_label(period)} · {_VALUE_LABELS[period.risk_type]} "
        f"{value}{qualifier}\n"
        f"Действие: {html.escape(period_action(period))}"
    )
